...
def amazon_scrap_ac(link, page):
    ...
    c=conn.cursor()
    # c.execute(""" create table amazon(website text, url text, title text,price text, company text, image text) """)
        ...
        if r.status_code==200:
            ...
            for product in products:
                ...
                if "none" not in title:
                    ...
                    c.execute("insert into amazon values(?,?,?,?,?,?)", ("Amazon",link,getName(title),price,prod_company, image))
